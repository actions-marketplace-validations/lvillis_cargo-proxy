FROM rust:1.94.0-alpine3.20 AS builder
...
COPY --from=builder /opt/app/target/x86_64-unknown-linux-musl/release/cargo-proxy /usr/local/bin/cargo-proxy

ENTRYPOINT ["/usr/local/bin/cargo-proxy"]
